...
async function main(): Promise<void> {
  ...
  const templates = {
    nodejs: await db.goldenPathTemplate.upsert({
      ...
      create: {
        ...
        steps: [
          { id: 'clone', name: 'Clone repository', action: 'fetch:template', input: {} },
          {
            id: 'provision',
    ...
    go: await db.goldenPathTemplate.upsert({
      ...
      create: {
        ...
        parameters: {
          moduleName: { type: 'string', description: 'Go module name' },
      ...
      create: {
        ...
        steps: [
          { id: 'clone', name: 'Clone repository', action: 'fetch:template', input: {} },
          {
            ...
            name: 'Setup database',
            action: 'run:script',
            input: { command: 'npx prisma migrate dev' },
          },
        ],
      },
    }),
  ...
  const apps = await Promise.all([
    db.application.create({
      data: {
        ...
        status: 'active',
        templateId: templates.nodejs.id,
        teamId: teams.rocket.id,
    ...
    db.application.create({
      data: {
        ...
        status: 'active',
        templateId: templates.fullstack.id,
        teamId: teams.photon.id,
    ...
    db.application.create({
      data: {
        name: 'audit-logger',
        description: 'Audit log ingestion and query service',
        status: 'active',
        templateId: templates.python.id,
        teamId: teams.photon.id,
        ownerId: users.carole.id,
    ...
    db.application.create({
      data: {
        name: 'idp-console',
        description: 'Kubernal IDP web console (legacy)',
        status: 'active',
        templateId: templates.react.id,
        teamId: teams.platform.id,
  ...
  const violations = [
    {
      ...
      severity: 'critical',
      message: 'Container securityContext.privileged must be false',
      resource: 'Deployment/payment-api',
    },
    {
      policyId: '',
      policyName: 'read-only-rootfs',
      severity: 'high',
    ...
    {
      policyId: '',
      policyName: 'resource-limits',
      severity: 'medium',
      message: 'Container resource limits are required',
      resource: 'Deployment/customer-portal',
    },
    {
      policyId: '',
      policyName: 'no-latest-tag',
      severity: 'high',
      message: 'Container image tag must not be "latest"',
      resource: 'Deployment/scheduler-service',
    },
  ];
  ...
  for (let i = 0; i < 35; i++) {
    ...
    const completedAt = ['healthy', 'failed', 'rolled_back', 'cancelled'].includes(status)
    ...
    deploys.push(
      await db.deployment.create({
        data: {
          ...
          trigger,
          approvedById:
            env.requiresApproval && (status === 'healthy' || status === 'deploying')
              ? users.admin.id
              : null,
          startedAt,
          createdAt,
              ...
              ? [
                  {
                    ...
                    size: `${(Math.random() * 50 + 10).toFixed(0)}MB`,
                  },
    ...
    const numPipelines =
      dep.status === 'cancelled'
        ...
        : dep.status === 'pending'
          ...
          : dep.status === 'building'
            ? 1
            : Math.min(3, pipelineNames.length);
  ...
  const policies = [
    ...
    },
    {
      ...
      description: 'Requires read-only root filesystem for containers',
      category: 'security',
      severity: 'high',
      engine: 'kyverno',
      enabled: true,
    ...
    {
      name: 'cost-aware-deployments',
      description: 'Production deployments must have cost allocation tags',
      category: 'cost',
      severity: 'low',
      engine: 'opa',
      enabled: false,
    ...
    {
      name: 'network-policy-enforced',
      description: 'All namespaces must have a default deny NetworkPolicy',
      category: 'compliance',
      severity: 'critical',
      engine: 'kyverno',
      enabled: true,
