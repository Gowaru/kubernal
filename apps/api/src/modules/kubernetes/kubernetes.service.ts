...
async function tryK8s<T>(realCall: () => Promise<T>, context: string): Promise<T> {
  try {
    return await realCall();
  } catch (err) {
    logger.warn({ context, error: String(err) }, 'K8s call failed');
    throw err;
  }
}
...
export const kubernetesService = {
    ...
    return tryK8s(async () => {
      const mapItem = (item: {
        ...
        const containers = (item.spec?.containers ?? []).map((c) => {
          const cs = (item.status?.containerStatuses ?? []).find((s) => s.name === c.name);
          ...
          return {
            ...
            reason: (cs?.state?.waiting?.reason ?? cs?.state?.terminated?.reason ?? null) as
              | string
              | null,
          };
        });
  ...
  async listServices(namespace: string): Promise<K8sService[]> {
    return tryK8s(async () => {
      const mapItem = (svc: {
        metadata?: { name?: string; namespace?: string; creationTimestamp?: Date | null };
        spec?: {
          type?: string;
          clusterIP?: string;
          ports?: Array<{
  ...
  async listEvents(namespace: string, limit: number): Promise<K8sEvent[]> {
    return tryK8s(async () => {
      const mapItem = (evt: {
        ...
        count: evt.count ?? 1,
        lastTimestamp:
          evt.lastTimestamp?.toISOString() ??
          evt.metadata?.creationTimestamp?.toISOString() ??
  ...
  async getArgoStatus(application: string): Promise<ArgoAppStatus | null> {
    return tryK8s(async () => {
      const res = (await customObjectsApi
      ...
      if (!res) return null;
      const syncStatus =
        (res.status as { sync?: { status?: string } } | undefined)?.sync?.status ?? 'Unknown';
      const healthStatus =
        (res.status as { health?: { status?: string } } | undefined)?.health?.status ?? 'Unknown';
      const source = (
        res.spec as
  ...
  async setAutoSync(
    application: string,
    enabled: boolean,
  ): Promise<{ success: boolean; message: string; autoSync: boolean }> {
    return tryK8s(async () => {
      await customObjectsApi.patchNamespacedCustomObject({
        group: 'argoproj.io',
        version: 'v1alpha1',
      ...
      return {
        success: true,
        message: enabled ? 'Auto-sync activé' : 'Auto-sync désactivé',
        autoSync: enabled,
      };
    }, 'setAutoSync');
  },
    ...
    return tryK8s(async () => {
      ...
      return (res.items ?? []).map((item: Record<string, unknown>) => {
        ...
        const memCurrent = currentMetrics.find(
          (m) =>
            m.type === 'Resource' && (m.resource as Record<string, unknown>)?.name === 'memory',
        );
        const getTarget = (m: Record<string, unknown> | undefined): number | null => {
          if (!m) return null;
          const target = (m.resource as Record<string, unknown>)?.target as
        ...
        const getCurrent = (m: Record<string, unknown> | undefined): number | null => {
          if (!m) return null;
          const current = (m.resource as Record<string, unknown>)?.current as
            | Record<string, unknown>
            | undefined;
  ...
  async listAllHPA(): Promise<K8sHPAStatus[]> {
    return tryK8s(async () => {
  ...
  async listClaims(namespace: string): Promise<CrossplaneClaim[]> {
    return tryK8s(async () => {
      ...
      return (res.items ?? []).map((item: Record<string, unknown>) => {
        const meta = (item.metadata ?? {}) as Record<string, unknown>;
        const spec = (item.spec ?? {}) as Record<string, unknown>;
        const itemStatus = (item.status ?? {}) as Record<string, unknown>;
        const conditions = Array.isArray(itemStatus.conditions)
          ? (itemStatus.conditions as Array<Record<string, unknown>>)
          : [];
        const readyCond = conditions.find((c) => (c.type as string) === 'Ready');
        return {
          id: (meta.uid as string) ?? 'unknown',
          name: (meta.name as string) ?? 'unknown',
          namespace: (meta.namespace as string) ?? namespace,
          kind: ((item.kind as string) ?? 'Composite') as CrossplaneClaimKind,
          status: ((readyCond?.status as string) === 'True'
            ? 'Ready'
            : 'Binding') as CrossplaneClaimStatus,
          class: ((spec.compositionRef as Record<string, unknown>)?.name as string) ?? 'default',
          message: (readyCond?.message as string) ?? null,
          boundAt: (meta.creationTimestamp as string) ?? null,
          endpoint:
            ((
              (itemStatus.atProvider as Record<string, unknown>)?.connectionDetails as Record<
                string,
                unknown
              >[]
            )?.[0]?.endpoint as string) ?? null,
        } as CrossplaneClaim;
      });
    }, 'listClaims');
  },
    ...
    return tryK8s(async () => {
      ...
      if (deleteService) {
        try {
          ...
          logger.warn(
            { namespace, name, error: String(err) },
            'deleteNamespacedService best-effort failed',
          );
