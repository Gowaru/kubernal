...
import { createApplicationSchema, updateApplicationSchema } from "../modules/application/application.schema.js";
import { createDeploymentSchema, transitionStatusSchema, approveDeploymentSchema, recordViolationsSchema } from "../modules/deployment/deployment.schema.js";
...
describe("User schemas", () => {
  describe("createUserSchema", () => {
    ...
    it("rejects invalid role", () => {
      const result = createUserSchema.safeParse({ email: "test@test.com", name: "Test", role: "superadmin" });
      expect(result.success).toBe(false);
    });
...
describe("Application schemas", () => {
  ...
  it("rejects invalid URL", () => {
    const result = createApplicationSchema.safeParse({
      ...validApp, repositoryUrl: "not-a-url",
    });
    expect(result.success).toBe(false);
  });
...
describe("Deployment schemas", () => {
  ...
  it("rejects invalid trigger", () => {
    expect(createDeploymentSchema.safeParse({ ...validDeployment, trigger: "invalid" }).success).toBe(false);
  });

  it("rejects empty version", () => {
    expect(createDeploymentSchema.safeParse({ ...validDeployment, version: "" }).success).toBe(false);
  });

  it("rejects invalid UUID for applicationId", () => {
    expect(createDeploymentSchema.safeParse({ ...validDeployment, applicationId: "bad" }).success).toBe(false);
  });

  describe("transitionStatusSchema", () => {
    it("accepts valid statuses", () => {
      for (const status of ["building", "deploying", "healthy", "failed", "rolled_back", "cancelled"]) {
...
describe("Team schema", () => {
  it("accepts valid team", () => {
    expect(createTeamSchema.safeParse({ name: "platform", namespacePrefix: "plat" }).success).toBe(true);
  });

  it("accepts optional fields", () => {
    expect(createTeamSchema.safeParse({
      name: "platform", description: "Platform team", namespacePrefix: "plat",
    }).success).toBe(true);
  });
...
describe("Template schema", () => {
  it("accepts valid template", () => {
    expect(createTemplateSchema.safeParse({
      name: "nodejs-backend", category: "backend", description: "Node.js backend", repository: "https://github.com/example",
    }).success).toBe(true);
  });

  it("rejects invalid category", () => {
    expect(createTemplateSchema.safeParse({
      name: "test", category: "invalid", description: "test", repository: "https://github.com/example",
    }).success).toBe(false);
  });
});
...
describe("Environment schema", () => {
  it("accepts valid environment", () => {
    expect(createEnvironmentSchema.safeParse({
      name: "dev", type: "dev", applicationId: uuid(), namespace: "my-app-dev", clusterName: "kubernal",
    }).success).toBe(true);
  });

  it("rejects invalid type", () => {
    expect(createEnvironmentSchema.safeParse({
      name: "dev", type: "production", applicationId: uuid(), namespace: "test",
    }).success).toBe(false);
  });
});
...
describe("Pipeline schema", () => {
  it("accepts valid pipeline", () => {
    expect(createPipelineSchema.safeParse({
      deploymentId: uuid(), name: "build-and-test",
    }).success).toBe(true);
  });
...
describe("Policy schema", () => {
  ...
  it("rejects invalid category", () => {
    expect(createPolicySchema.safeParse({
      name: "test", description: "test", category: "invalid", severity: "high", engine: "kyverno",
    }).success).toBe(false);
  });
});
