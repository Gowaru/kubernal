...
export function generateSecret(): string {
  return (
    'whsec_' +
    createHmac('sha256', Date.now().toString() + Math.random().toString())
...
export function parseGitHubPush(body: Record<string, unknown>): ParsedPushEvent | null {
  ...
  const repository =
    ((body['repository'] as Record<string, unknown> | undefined)?.['full_name'] as
      | string
      | undefined) ?? '';
  const sender =
...
export function parseGitLabPush(body: Record<string, unknown>): ParsedPushEvent | null {
  ...
  const after = (body['after'] as string | undefined) ?? '';
  const repository =
    ((body['project'] as Record<string, unknown> | undefined)?.['path_with_namespace'] as
      | string
      | undefined) ?? '';
  const userName =
    (body['user_name'] as string | undefined) ??
    (body['user_username'] as string | undefined) ??
    '';
  const branch = ref.replace('refs/heads/', '');
  if (!ref || !after) return null;
...
export function parseBitbucketPush(body: Record<string, unknown>): ParsedPushEvent | null {
  ...
  const changes = (pushData?.changes ?? []) as Array<{
    new?: { target?: { hash?: string }; name?: string };
  }>;
  ...
  const repository =
    ((body['repository'] as Record<string, unknown> | undefined)?.['full_name'] as
      ...
      | undefined) ?? '';
  const sender =
    ((body['actor'] as Record<string, unknown> | undefined)?.['display_name'] as
      | string
      | undefined) ?? '';
