...
export function PodLogDrawer({ pod, onClose }: PodLogDrawerProps): JSX.Element {
  ...
  const {
    ...
    transport,
    isFollowing,
    setFollow: setHookFollow,
    clear,
  } = usePodLogs(pod?.name ?? '', pod?.namespace ?? '', { tailLines: 200, enabled: !!pod, follow });
  ...
  return (
    <AnimatePresence>
      {pod && (
        <>
          <div className="fixed inset-0 z-40 bg-overlay/50 backdrop-blur-sm" onClick={onClose} />
          <motion.div
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
          ...
          >
            <div className="flex items-center justify-between px-4 py-2 bg-secondary border-t border-x border-border rounded-t-lg">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={cn('h-2 w-2 rounded-full', STATUS_DOT[pod.status] ?? 'bg-k8s-unknown')}
                />
                ...
                >
                  {transport === 'ws' ? (
                    ...
                    <WifiOff
                      className={cn(
                        'h-3 w-3',
                        transport === 'poll' ? 'text-amber-500' : 'text-muted-foreground',
                      )}
                    />
                  )}
