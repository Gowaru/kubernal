...
export function useTransitionDeployment(): UseMutationResult<
  Deployment,
  Error,
...
> {
  ...
  return useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const { data } = await apiClient.post<{ data: Deployment }>(`/deployments/${id}/transition`, {
        status,
      });
      return data.data;
    },
...
export function useApproveDeployment(): UseMutationResult<
  Deployment,
  Error,
...
export function usePromoteDeployment(): UseMutationResult<
  Deployment,
  Error,
  { id: string; targetEnv: 'staging' | 'prod' }
> {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, targetEnv }: { id: string; targetEnv: 'staging' | 'prod' }) => {
      const { data } = await apiClient.post<{ data: Deployment }>(`/deployments/${id}/promote`, {
        targetEnv,
      });
      return data.data;
    },
